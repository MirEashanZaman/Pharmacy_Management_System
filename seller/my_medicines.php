...
if (isset($_GET['delete'])) {
    ...
    $conn->query("UPDATE medicines SET is_active=0 WHERE id=$mid AND salesperson_id={$user['id']}");
    $success = 'Medicine removed from listing.';
}

$meds = $conn->query("SELECT * FROM medicines WHERE salesperson_id={$user['id']} ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="card">
        <div class="table-container">
            <table>
                ...
                <?php $sold=$conn->query("SELECT SUM(oi.quantity) as s FROM order_items oi JOIN orders o ON oi.order_id=o.id WHERE oi.medicine_id={$m['id']} AND o.status='delivered'")->fetch_assoc()['s']??0; ?>
                <tr>
                    <td>
                        ...
                        <?php else: ?>
                            <div style="width:55px;height:55px;border-radius:10px;background:#f0f4ff;display:flex;align-items:center;justify-content:center;font-size:1.8rem;">💊</div>
                        <?php endif; ?>
                    </td>
                    <td>
                        <strong><?= htmlspecialchars($m['name']) ?></strong>
                        <br><small class="text-muted"><?= htmlspecialchars($m['generic_name']) ?></small>
                        <br><small class="text-muted">🏭 <?= htmlspecialchars($m['brand']) ?></small>
                    ...
                    <td>
                        <div style="display:flex; gap:6px; flex-wrap:wrap;">
                            <a href="edit_medicine.php?id=<?= $m['id'] ?>" class="btn btn-warning btn-sm">✏️ Edit</a>
                            <?php if($m['is_active']): ?>
                            <a href="?delete=<?= $m['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Remove this medicine from listing?')">🗑️ Remove</a>
                            <?php endif; ?>
                            <a href="../medicine_detail.php?id=<?= $m['id'] ?>" class="btn btn-secondary btn-sm">👁️</a>
                        </div>
                    </td>
                </tr>
                <?php endwhile; ?>
