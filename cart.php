...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['update_qty'])) {
    ...
    if ($qty<=0) {
        $conn->query("DELETE FROM cart WHERE id=$cartId AND user_id={$user['id']}");
    } else {
        $conn->query("UPDATE cart SET quantity=$qty WHERE id=$cartId AND user_id={$user['id']}");
    }
...
$items = $conn->query("SELECT c.*, m.name, m.price, m.stock, m.image, m.unit, m.brand FROM cart c JOIN medicines m ON c.medicine_id=m.id WHERE c.user_id={$user['id']}");
$total = 0;
$itemsList = [];
while($row=$items->fetch_assoc()) { $itemsList[]=$row; $total+=$row['price']*$row['quantity']; }
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['place_order'])) {
    ...
    if (empty($delAddress)||empty($delDiv)||empty($delDist)||empty($delPhone)) {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO orders (user_id,total_amount,delivery_address,delivery_division,delivery_district,delivery_upazila,delivery_phone,payment_method) VALUES (?,?,?,?,?,?,?,?)");
            $stmt->bind_param("idssssss", $user['id'],$total,$delAddress,$delDiv,$delDist,$delUpaz,$delPhone,$payMethod);
            $stmt->execute();
            $orderId = $conn->insert_id;

            foreach($itemsList as $item) {
                $stmt2 = $conn->prepare("INSERT INTO order_items (order_id,medicine_id,quantity,price) VALUES (?,?,?,?)");
                $stmt2->bind_param("iiid", $orderId, $item['medicine_id'], $item['quantity'], $item['price']);
                $stmt2->execute();
                
                $conn->query("UPDATE medicines SET stock=stock-{$item['quantity']} WHERE id={$item['medicine_id']} AND stock>={$item['quantity']}");
            }
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="cart-item">
                    ...
                    <a href="cart.php?remove=<?= $item['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Remove this item?')">🗑️</a>
                </div>
                <?php endforeach; ?>
