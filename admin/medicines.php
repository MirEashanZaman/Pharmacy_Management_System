...
if (isset($_GET['toggle'])) {
    ...
    $conn->query("UPDATE medicines SET is_active = NOT is_active WHERE id=$mid");
    $success = 'Medicine status updated.';
}

$meds = $conn->query("SELECT m.*, u.name as seller_name FROM medicines m LEFT JOIN users u ON m.salesperson_id=u.id ORDER BY m.created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <table>
                ...
                <tr>
                    <td>
                        ...
                        <?php else: ?>
                            <div style="width:50px;height:50px;border-radius:8px;background:#f0f4ff;display:flex;align-items:center;justify-content:center;font-size:1.5rem;">💊</div>
                        <?php endif; ?>
                    </td>
